    ...
    <section id="how" className="relative bg-background py-28 sm:py-36 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        >
          {STEPS.map((s) => (
            <motion.div
              key={s.n}
              variants={item}
              className="rounded-3xl border border-foreground/10 bg-white p-8 shadow-sm"
            >
        ...
        <motion.div
          ...
          className="rounded-3xl border border-foreground/10 bg-white shadow-xl shadow-primary/5 overflow-hidden"
        >
          <div className="px-6 sm:px-10 pt-8 pb-4 flex items-center justify-between flex-wrap gap-2">
            ...
            <span className="font-mono text-xs px-3 py-1 rounded-full bg-primary/10 text-primary">
          ...
          <div className="grid lg:grid-cols-[2fr_3fr] gap-8 p-6 sm:p-10 pt-4">
              ...
              <div className="rounded-2xl overflow-hidden border border-foreground/10">
                <div className="relative aspect-video bg-gradient-to-br from-[#7db8ec] via-[#2b7de0] to-[#0e2233] flex items-center justify-center">
                  ...
                  <span className="relative w-16 h-16 rounded-full bg-white/90 flex items-center justify-center shadow-lg">
                    <Play className="w-7 h-7 text-foreground translate-x-0.5" fill="currentColor" aria-hidden="true" />
                  </span>
            ...
            <div className="flex flex-col gap-6">
              {/* Sentiment */}
              <motion.div variants={item}>
                <p className="text-sm font-semibold text-foreground mb-3">Sentiment</p>
              ...
              {/* Themes */}
              <motion.div variants={item}>
                <p className="text-sm font-semibold text-foreground mb-3">Top themes</p>
                <div className="flex flex-wrap gap-2">
                  {THEMES.map((t) => (
                    <span
                      key={t.label}
                      className="px-3 py-1.5 rounded-full bg-primary/8 border border-primary/15 text-sm text-foreground/80"
                    >
                      {t.label} <span className="font-mono text-xs text-primary ml-1">{t.count}</span>
                    </span>
                  ))}
                </div>
              </motion.div>
              ...
              {/* Quotes */}
              <motion.div variants={item}>
                <p className="text-sm font-semibold text-foreground mb-3">Quotes worth keeping</p>
